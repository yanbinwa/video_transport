...
def get_video_download_url(url):
    data = get_video_audio_list_download_url(url);
    # 如何判断
    data_list = get_video_audio_list_download_url(url);
    # 如何判断: asr != None and fileSize != None，取fileSize最大的一个作为结果
    data_list = [data for data in data_list if data.get("asr", None) is None and data.get("filesize", None) is not None]
...
def get_audio_download_url(url):
    data_list = get_video_audio_list_download_url(url);
    # 如何判断: asr != None and fileSize != None，取fileSize最大的一个作为结果
    data_list = [data for data in data_list if data.get("asr", None) is not None and data.get("filesize", None) is not None]
    return max(data_list, key=lambda x: x["filesize"])["url"]
