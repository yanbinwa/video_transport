...
from core.utils.video.video_spilt_with_llm import srt_spilt_by_llm, srt_spilt_by_llm_v2
...
class VideoSpiltWithLLMTest(unittest.TestCase):

    def test_video_spilt_with_llm(self):
        time_ranges = srt_spilt_by_llm(srt_content)
        print(time_ranges)


    def test_video_spilt_with_llm_v2(self):
        srt_file_path = os.path.join(project_root, 'file', 'v1.srt')
        time_ranges = srt_spilt_by_llm_v2(srt_file_path)
        print(time_ranges)
